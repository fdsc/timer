...
Требует установленных пакетов sox, tk, zenity.
Часто эти пакеты уже установлены, кроме sox.

Для установки просто скопируйте папку "python" из этого репозитория на ваш компьютер.
Запуск: python3 main.py (команда подаётся из папки python)


Поддержка JavaScript напоминателя прекращена (лежит в корневой директории).
# Далее идёт старая справка по JavaScript-таймеру.
